import { createPostRepo, getAllPostRepo, getByIdRepo } from "../repositaries/postRepo";
import { makeResponse } from "../utilities/response";

export const createPostService = async (body) => {
    const {title, description} = body
    try {
        if (!title || !description) return { status: 400, message: 'Post title and the description required' };
